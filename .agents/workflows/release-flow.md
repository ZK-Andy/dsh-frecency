...
4. **Release 核验**：标记 = Latest、非 prerelease/draft；正文为 release-notes.sh 结构化输出（新增/修复/优化/文档/构建·CI·其他五节，每条带短 sha），且**标题与 Changelog 链接指发布 tag**——重放 `bash scripts/release-notes.sh "" <tag>` 与 `gh release view <tag> --json body --jq .body` 比对（差异允许 GitHub API 追加的尾部空行）；npm 版本号发布后不可重发——宁跳版本号也不重打已发布的 tag。
...
> 本表承载**发版链路**专属坑位（发布动作、tag/Release、收录）；产品与市场类踩坑仍归 [docs/cookbook.md](../../docs/cookbook.md) 的市场与发布节。
